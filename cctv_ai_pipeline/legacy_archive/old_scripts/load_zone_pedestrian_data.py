...
def main():
    ...
    for idx, group in enumerate(groups_to_process):
        ...
        if args.realtime_batch:
            ...
            try:
                from realtime_cctv_batch_pipeline import RealtimeBatchPipeline
                ...
                for zone_id, records in batch_results.items():
                    ...
                    for r in records:
                        ...
                        rec = {
                            ...
                            "pixels_json": json.dumps([{"x": c["px"], "y": c["py"]} for c in r["coordinates"]]),
                            "bev_xyz_json": json.dumps([{"x": c["bev_x"], "y": c["bev_y"], "z": 0.0} for c in r["coordinates"]]),
        ...
        def run_zone_04(zone):
            ...
            print(f"[Zone {zone}] 파일 시작: {os.path.basename(v_path)}")
            script_path = os.path.join(BASE_DIR, "04_video_to_bev_CSR.py")
            env = os.environ.copy()
            env.update(env_04)
            env["PYTHONIOENCODING"] = "utf-8"
            t0 = time.time()
